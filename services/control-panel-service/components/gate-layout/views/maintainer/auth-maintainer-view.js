...
const form_factories_1 = require("@sotaoi/client-forms/form-factories");
...
class AuthMaintainerView extends components_1.ViewComponent {
    ...
    init({ results, props }) {
        const authMaintainerFormConstructor = form_factories_1.FormConstructor({
            email: input_field_1.InputField.input(''),
            password: input_field_1.InputField.input(''),
        }, results.validations);
        const Form = form_factories_1.AuthFormFactory(new artifacts_1.Artifacts(), 'sotaoi-maintainer', authMaintainerFormConstructor, 'test:password:email:username');
        Form.init();
